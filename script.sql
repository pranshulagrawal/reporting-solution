CREATE PROCEDURE sp_get_dashboard_consolidated (
    @start_date DATE,
    @end_date DATE
)
AS
BEGIN
    -- Set the date format for this session if needed
    SET TEMPORARY OPTION DATE_FORMAT = 'YYYY-MM-DD';

    -- 1. Query for KPI Cards
    SELECT
        COUNT(CASE WHEN CONVERT(DATE, b.created_at) = TODAY(*) THEN 1 END) AS todaysBreaks,
    ...
    FROM
        break_data b
    WHERE
        CONVERT(DATE, b.created_at) BETWEEN @start_date AND @end_date;
